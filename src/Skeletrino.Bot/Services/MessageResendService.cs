...
public class MessageResendService : IMessageResendService
{
    ...
    private readonly Regex _messagePattern =
        new(@"(?<!\\)https?:\/\/(?:ptb\.|canary\.)?discord\.com\/channels\/(\d+)\/(\d+)\/(\d+)", RegexOptions.Compiled);

    private readonly DiscordEmoji _redCrossEmoji;
    private readonly ILogger<MessageResendService> _logger;

    public MessageResendService(DiscordClient client, ILogger<MessageResendService> logger)
    {
        _client = client;
        _logger = logger;
